...
// =====================================================================================================================================================================
double last_mu_used;
int sba_damping_iters;
double sba_time_total, sba_time_solve, sba_time_system;
...
//double time_initialization = (double)(t_fin - t_ini) / CLOCKS_PER_SEC;

    /* determine increment using adaptive damping */
// Nota: Aqui se resuelve el sistema reducido de segundo nivel.
    while(1){
      /* augment U, V */
      for(j=mcon; j<m; ++j){
        ptr1=U + j*Usz; // set ptr1 to point to U_j
...
// Nota: computa la matriz S reducida de sistema. Primero se calcula la parte superior triangular, y luego se copia.
...
// -----------------------------
	...
	{
	dimS = m*cnp;
	copyS = (double*) malloc(sizeof(double)*dimS*dimS);
	int i;
...
// -----------------------------
	...
	// Nota: aqui se resuelve el sistema lineal 'S xInc = E'
clock_t t0_solve = clock();
...
// Nota: no destruimos 'S', sino que la guardamos
...
time_total = t_end - t_init;

sba_time_total = (double)time_total / CLOCKS_PER_SEC,
sba_time_solve = (double)time_solve / CLOCKS_PER_SEC;
sba_time_system = sba_time_total - sba_time_solve;

printf("**** time total = %f\n", sba_time_total);
printf("**** time solve = %f\n", sba_time_solve);
printf("**** time not solve = %f\n", sba_time_system);
printf("**** mu = %.12f\n", mu);
last_mu_used = mu;
  return retval;
}
